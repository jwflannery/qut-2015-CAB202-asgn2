	...
	while(1){
		...
		while (state == 4) {
			...
			// Wait until the 'debugger' is attached...
				draw_string(10, 24, "Please Connect");
				show_screen();
			...
			if (USE_SERIAL){
				if (curr_char == 'w' && player_y > 11){
					player_y -= 2;
				}
				if (curr_char == 's' && player_y + 8 < 48){
					player_y += 2;
				}
				if (curr_char == 'a' && player_x > 0){
					player_x -= 2;
				}
				if (curr_char == 'd' && player_x + 8 < 84){
					player_x += 2;
				}
			}
